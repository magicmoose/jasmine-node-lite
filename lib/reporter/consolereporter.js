...
function configureEvents(self, reportDispatcher) {
    function colored(displayType, indent, str) {
        ...
        return self.showColors ? (getIndent(indent) + displayType.colorCode + str + _Display.none.colorCode) : (getIndent(indent) + displayType.text + str);
    }
    ...
    function reportFailures() {
        ...
        for (var i = 0; i < self.failedSpecs.length; i++) {
            ...
            for (var j = 0; j < failedSpec.failedExpectations.length; j++) {
                ...
                self.println(colored(_Display.error, 1, failedExpectation.message));
                if (self.stackTrace) {
    ...
    function onJasmineDone() {
        ...
        self.println(colored(self.failureCount === 0 ? _Display.ok : _Display.error, 0 /*indent*/ , message));
    ...
    function onSuiteStarted(result) {
        if (self.verbose) {
            self.println(colored(_Display.info, self.indent, result.description));
        }
        self.indent++;
    }
    ...
    function onSpecDone(result) {
        ...
        if (result.status === 'pending') {
            ...
            if (self.verbose) {
                self.println(colored(_Display.warning, self.indent, result.description));
            } else {
                self.print('*');
            }
        }
        ...
        if (result.status === 'passed') {
            if (self.verbose) {
                self.println(colored(_Display.ok, self.indent, result.description));
            } else {
                self.print('.');
            }
        }
        ...
        if (result.status === 'failed') {
            ...
            if (self.verbose) {
                self.println(colored(_Display.error, self.indent, result.description));
            } else {
                self.print('F');
            }
        }
